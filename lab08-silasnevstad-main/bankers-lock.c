...
void *disburse(void *arg) {
  ...
  for (i = 0; i < N_ROUNDS; i++) {
    ...
    // lock in order
    pthread_mutex_lock(&accts[MIN(from, to)].lock);
    pthread_mutex_lock(&accts[MAX(from, to)].lock);
    ...
    pthread_mutex_unlock(&accts[MAX(from, to)].lock);
    pthread_mutex_unlock(&accts[MIN(from, to)].lock);
  }
  return NULL;
}
